...
def test_FADD(n: int):
    for _ in range(5):
        ...
        expected = (a + b) % (2**n)
        ans1 = eval_qsharp(f"QuantumArithmetic.PG2012Test.TestFADD({n},{a},{b})")
        assert ans1 == expected
        ans2 = eval_qsharp(f"QuantumArithmetic.PG2012Test.TestFADD2({n},{a},{b})")
...
def test_FMAC(n: int):
    for _ in range(5):
        ...
        print(n, a, b, x)
        ans = eval_qsharp(f"QuantumArithmetic.PG2012Test.TestFMAC({n},{a},{b},{x})")
...
def test_GMFDIV(n: int):
    for _ in range(5):
        ...
        assert 0 <= a // b < 2**n
        q, r = eval_qsharp(f"QuantumArithmetic.PG2012Test.TestGMFDIV({n},{a},{b})")
        assert (q, r) == (a // b, a % b)
...
def test_MUL_MOD():
    for i in range(1, 7):
        ans = eval_qsharp(f"QuantumArithmetic.PG2012Test.TestFMUL_MOD2(3,5,{i},7)")
        assert ans == (i * 5) % 7


@pytest.mark.parametrize("n,a,b,N", [(2, 2, 2, 3), (3, 6, 5, 7)])
def test_EXP_MOD(n, a, b, N):
    op = f"QuantumArithmetic.PG2012.EXP_MOD(_,_,{a}L,{N}L)"
    tester = ArithmeticOpTester(op, [n, n])
    assert tester.run([b, 0]) == [b, (a**b) % N]
